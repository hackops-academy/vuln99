...
BASE_CSS = """
  ...
  *{box-sizing:border-box;}
  ...
  nav .navlinks a{color:var(--text-dim); text-decoration:none; font-size:14px; font-weight:500; padding:6px 2px; border-bottom:2px solid transparent;}
  nav .navlinks a:hover{color:var(--accent);}
  nav .navlinks a.active{color:var(--text); border-bottom-color:var(--accent);}
  ...
  nav .navlinks a.lab-link.active{border-bottom:none; background:rgba(63,208,201,0.22);}
  nav .spacer{flex:1;}
  .nav-toggle{display:none; background:transparent; border:1px solid var(--line); color:var(--text); padding:8px 10px; border-radius:8px; font-size:16px;}
  ...
  .product:hover{border-color:rgba(63,208,201,0.4); box-shadow:var(--shadow); transform:translateY(-2px);}
  .product{transition:transform 0.15s ease, border-color 0.15s ease;}
  .product .thumb{height:112px; display:flex; align-items:center; justify-content:center; font-size:40px; position:relative; border-bottom:1px solid var(--line);
    background:
      radial-gradient(120px 80px at 30% 20%, rgba(63,208,201,0.16), transparent 60%),
      radial-gradient(120px 80px at 80% 90%, rgba(124,140,255,0.14), transparent 60%),
      linear-gradient(160deg,var(--surface-2),var(--bg-alt));}
  .product .thumb::after{content:""; position:absolute; inset:0; box-shadow:inset 0 0 0 1px rgba(255,255,255,0.03);}
  ...
  @media (max-width:720px){
    ...
    nav .navlinks{display:none; width:100%; flex-direction:column; gap:2px; order:5;}
    nav.nav-open .navlinks{display:flex;}
    nav .navlinks a{padding:10px 4px; border-bottom:1px solid var(--line-soft);}
    .hero{flex-direction:column; align-items:flex-start;}
    .footer-grid{grid-template-columns:1fr 1fr;}
  }
  /* ---- /lab attack cards ---- */
  .atk-stack{display:flex; flex-direction:column; gap:10px;}
  .atk-card{background:var(--surface); border:1px solid var(--line); border-radius:var(--radius); overflow:hidden;}
  .atk-card[open]{border-color:rgba(63,208,201,0.35);}
  .atk-summary{list-style:none; cursor:pointer; display:flex; align-items:center; justify-content:space-between; gap:14px; padding:14px 16px;}
  .atk-summary::-webkit-details-marker{display:none;}
  .atk-summary:hover{background:var(--surface-2);}
  .atk-summary-main{display:flex; flex-direction:column; gap:3px; min-width:0;}
  .atk-summary-main b{font-size:14px;}
  .atk-desc{max-width:520px;}
  .atk-summary-side{flex-shrink:0;}
  .atk-body{padding:0 16px 16px; display:flex; flex-direction:column; gap:10px; border-top:1px solid var(--line-soft);}
  .atk-tier{background:var(--bg-alt); border:1px solid var(--line-soft); border-radius:var(--radius-sm); padding:12px 14px; margin-top:12px; opacity:0.6;}
  .atk-tier.active{opacity:1; border-color:rgba(63,208,201,0.4); box-shadow:0 0 0 1px rgba(63,208,201,0.15) inset;}
  .atk-tier-head{display:flex; align-items:center; gap:8px; margin-bottom:8px;}
  .atk-current{font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:0.4px; color:var(--accent);}
  .atk-field{display:flex; align-items:flex-start; gap:8px; margin-bottom:6px; flex-wrap:wrap;}
  .atk-label{font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:0.4px; color:var(--text-faint); flex-shrink:0; padding-top:3px; width:60px;}
  .atk-field code{background:var(--surface); border:1px solid var(--line); border-radius:6px; padding:4px 8px; font-size:12px; color:var(--text); word-break:break-all;}
  .atk-payload{color:var(--accent);}
  .atk-note{font-size:12px; color:var(--text-dim); line-height:1.5; margin-top:2px;}
  .copy-btn{background:transparent; border:1px solid var(--line); color:var(--text-dim); font-size:10.5px; font-weight:700; text-transform:uppercase; padding:3px 8px; border-radius:6px;}
  .copy-btn:hover{border-color:var(--accent); color:var(--accent); opacity:1;}
  .atk-actions{display:flex; gap:10px; margin-top:2px;}
"""
...
<nav id="mainnav">
  ...
  <button type="button" class="nav-toggle" onclick="document.getElementById('mainnav').classList.toggle('nav-open')" aria-label="Menu">&#9776;</button>
  <div class="navlinks">
    <a href="/" class="{_navcls('/')}">Catalog</a>
    <a href="/category" class="{_navcls('/category')}">Categories</a>
    <a href="/recommend" class="{_navcls('/recommend')}">For you</a>
    <a href="/cart" class="{_navcls('/cart')}">Cart</a>
  </div>
  <span class="spacer"></span>
  <div class="navlinks">
    <a class="lab-link {_navcls('/lab')}" href="/lab">&#129514;&nbsp;Lab</a>
  </div>
